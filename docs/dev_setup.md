...
| 進化を確認 | スライム選択 → 「進化直前 (exp=490, fire=0.5)」→ ⚡ターン強制実行 |
| 分裂を確認 | スライム選択 → 「分裂直前 (exp=500, beast=0.71)」→ ⚡ターン強制実行（15%確率なので複数回） |
...
2. **進化確認**: 「進化直前 (exp=490, fire=0.5)」プリセット → 「⚡ ターン強制実行」→ ターンログに ★進化 が表示
3. **分裂確認**: 「分裂直前 (exp=500, beast=0.71)」プリセット → ターン強制実行を繰り返す（15%確率）→ スライム一覧が増える
